...
#do
#	echo "run $run_i"
	time ./word2vec -train $CORPUS_PATH -output word2vec_cbow$WINDOW_SIZE_run$run_i.txt -save-vocab vocab_word2vec_cbow$WINDOW_SIZE_run$run_i.txt -min-count $MIN_COUNT -cbow 1 -size 200 -window $WINDOW_SIZE -negative $N_NEGATIVE -hs 0 -sample 0 -threads $N_THREADS -binary $SAVE_AS_BIN -iter $N_ITER -prop-train 0.05
#done
